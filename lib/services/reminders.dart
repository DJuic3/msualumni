...
class NotifyHelper {
  ...
  cancelNotification(Account account) async {
    await flutterLocalNotificationsPlugin.cancel(account.id!);
  }
  cancelAllNotification() async {
    await flutterLocalNotificationsPlugin.cancelAll();
  }

  // scheduledNotification(int hour, int minutes, Account account) async {
  ...
  //     payload: '${account.fullname}|${account.country}|${account.date}|',
  //   );
  //   return 0;
  // }
  ...
  // }
  Future<void> _configureLocalTimeZone() async {
    tzdata.initializeTimeZones(); // Initialize the timezones (load the timezone data)

    final now = DateTime.now();
    final timeZoneOffset = now.timeZoneOffset;
    final timeZoneName = tz.getLocation(timeZoneOffset.inMinutes as String).toString();
